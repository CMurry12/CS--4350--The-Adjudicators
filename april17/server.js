...
app.post("/api/projects/review", async (req, res) => {
  ...
  console.log("Raw incoming review:", req.body);

  // Convert to numbers just in case they're sent as strings
  ...
  if (
    ...
    console.warn("❌ Missing or invalid fields:", { judgeId, projectId, creativity, impact, execution, feasibility, design });
...
// ✅ GET all projects submitted for a specific event (for judges to view and score)
...
app.post("/api/judge_event", async (req, res) => {
  ...
  try {
    await db.query(
      "INSERT INTO judge_event (judge_id, event_id) VALUES (?, ?)",
      [judgeId, eventId]
    );
